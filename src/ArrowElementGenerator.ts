...
type Attributes<T=any> = {[name:string]:T};

function extendAttributes(originalAttrs:Attributes,transformers:Attributes<(previousValue:any)=>any>={}){
...
export class ArrowElementGenerator<T> {
    ...
    constructor(type:string, attributes:Attributes,
                createTransform:(args:T, currentAttributes:Attributes)=>void){
        this.type = type;
     ...
     */
    create(requiredValue:T, attributes:Attributes<(previousValue:any)=>any>={}){
     ...
     */
    extend<T2>(attributes:Attributes={},
            newCreateTransform:(superTransform: (oldArgs: T, oldAttrs: Attributes) => void) =>
                (newArgs:T2, newAttrs:Attributes) => void,
            type?:string){
